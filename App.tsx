      ...
      {mode === 'landing' ? (
        <div className="min-h-screen w-full flex flex-col items-center justify-center p-6 relative">
          ...
          <div className="max-w-xl w-full text-center space-y-8 animate-in fade-in zoom-in duration-500">
            <h1 className="text-7xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-br from-white to-zinc-500">VibeStudio AI</h1>
          ...
          <div className={`flex-1 relative flex flex-col min-h-0 ${theme === 'dark' ? 'bg-[#080808]' : 'bg-zinc-200'} transition-colors duration-300`}>
            <div className="absolute top-4 inset-x-0 z-20 flex items-center justify-between px-6 pointer-events-none">
              <div className="flex gap-2 pointer-events-auto">
                <button onClick={() => setMode('landing')} className={`px-4 py-2 flex items-center gap-2 rounded-full backdrop-blur-md border shadow-sm transition-all ${theme === 'dark' ? 'bg-black/60 border-white/5 text-white' : 'bg-white/80 border-zinc-300 text-zinc-900'}`}><Layout className="w-4 h-4" /> Home</button>
